...
export default function ContestLeaderboard() {
  ...
  return (
    <div className="min-h-screen bg-[var(--raisin-black)] text-white font-sans selection:bg-[var(--dark-pastel-green)]/30 pb-20">
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-8 pt-12 relative z-10">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-6 mb-16">
          <div className="flex items-center gap-4 self-start md:self-auto">
            ...
            <div>
              <h1 className="text-3xl md:text-4xl font-bold tracking-tight font-serif">Leaderboard</h1>
              <p className="text-white/50 text-sm mt-1">Live rankings & performance stats</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 w-full md:w-auto">
            <div className="relative w-full md:w-auto flex-1">
              ...
              <input 
                ...
                className="w-full md:w-[300px] pl-10 pr-4 py-3 rounded-xl bg-white/5 border border-white/10 focus:border-[var(--dark-pastel-green)]/50 focus:bg-white/10 outline-none transition-all text-sm placeholder:text-white/20"
              />
            </div>
            <button 
              onClick={fetchLeaderboard}
              className="p-3 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 transition-colors text-white/70 hover:text-white hover:rotate-180 duration-500"
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <p className="text-white/40 animate-pulse">Calculating ranks...</p>
          </div>
        ) : error ? (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <button 
              onClick={fetchLeaderboard}
              className="px-6 py-2 rounded-lg bg-white/10 hover:bg-white/20 transition-colors font-bold text-sm"
            >
              Try Again
            </button>
          </div>
          ...
          <>
            ...
            {/* LIST SECTION */}
            <div className="bg-[#1e2022]/50 backdrop-blur-xl border border-white/5 rounded-3xl overflow-hidden shadow-2xl">
              <div className="grid grid-cols-12 gap-4 p-4 border-b border-white/5 text-xs font-bold uppercase tracking-wider text-white/40">
        ...
        <div className="mt-12 text-center opacity-20 hover:opacity-100 transition-opacity text-[10px] font-mono">
    ...
    >
      <div className="relative">
        ...
        <div className={`w-20 h-20 md:w-24 md:h-24 rounded-full p-1 bg-gradient-to-br ${colors[rank]} shadow-2xl`}>
          <div className="w-full h-full rounded-full bg-[#1e2022] flex items-center justify-center overflow-hidden border-4 border-[#1e2022]">
             <span className="text-2xl font-bold text-white/20">
               {initial}
             </span>
          </div>
        </div>
        <div className={`absolute -bottom-3 left-1/2 -translate-x-1/2 w-8 h-8 rounded-full bg-gradient-to-br ${colors[rank]} flex items-center justify-center text-white font-bold text-sm shadow-lg border-2 border-[#1e2022]`}>
          {rank}
        </div>
      </div>
...
function RankRow({ user, rank, index }) {
  ...
  return (
    ...
    >
      <div className="col-span-2 md:col-span-1 text-center font-mono font-bold text-white/50 group-hover:text-white transition-colors">
        #{rank}
      </div>
      ...
      <div className="col-span-6 md:col-span-5 flex items-center gap-3">
        ...
        <div>
          ...
          <div className="text-xs text-white/40 md:hidden">{user.accuracy}% Acc • {user.timeTaken}</div>
        </div>
      </div>

      <div className="col-span-2 md:col-span-2 text-center hidden md:flex items-center justify-center gap-1 text-sm text-white/60">
        <Target className="w-4 h-4 text-white/20" /> {user.accuracy}%
      </div>

      <div className="col-span-2 md:col-span-2 text-center hidden md:flex items-center justify-center gap-1 text-sm text-white/60">
        <Timer className="w-4 h-4 text-white/20" /> {user.timeTaken}
      </div>
